...
var db = require('./db.js'); // templat, db 모듈 가져오기

// 로그인 화면
router.get('/login', function (request, response) { // /login 경로로 접근했을때, 로그인 화면으로 이동한다. 
    var title = '로그인';
    var html = template.HTML(title,`
            ...
            <p>계정이 없으신가요?  <a href="/auth/register">회원가입</a></p> // 회원가입 링크로 이동시켜 준다.
        `, '');
    response.send(html);
});
...
router.get('/logout', function (request, response) {
    request.session.destroy(function (err) { // 세션 파괴
        response.redirect('/'); // 메인페이지로 이동한다
    });
});
